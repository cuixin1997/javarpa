...
export interface AppListData { count?: number; apps: { pkg: string; label?: string }[] }
export const deviceDebugTrigger = (id: any, kind: 'dump' | 'capture' | 'tap' | 'apps', data?: { x: number; y: number }) =>
  http.post(`/devices/${id}/debug/${kind}`, data)
export const deviceDebugLatest = (id: any, type: 'dump' | 'capture' | 'apps') =>
